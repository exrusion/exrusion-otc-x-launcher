...
function LaunchCard({ launch }: { launch: Launch }) {
  const pairSymbol = launch.pairSymbol || "Pair unavailable";
  const pair = pairs.find((item) => item.symbol.toLowerCase() === pairSymbol.toLowerCase());
  ...
  return <article className="group grid gap-5 rounded-[24px] border border-black/8 bg-white p-4 shadow-[0_14px_50px_rgba(22,63,42,.05)] sm:grid-cols-[88px_1fr_auto] sm:p-5">
    ...
    <div className="min-w-0">
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm text-[#69736b]"><span>by @{launch.creatorHandle}</span><span className="flex items-center gap-1.5">paired with {(launch.pairLogo || pair?.logo) && <img src={launch.pairLogo || pair?.logo} alt="" className="h-5 w-5 rounded-full" />} <b className="text-[#163f2a]">{pairSymbol}</b></span></div>
